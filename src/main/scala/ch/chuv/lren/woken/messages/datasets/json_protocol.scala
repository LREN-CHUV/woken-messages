...
trait DatasetsProtocol extends DefaultJsonProtocol with JsonEnums {
  ...
  implicit val AnonymisationLevelJsonFormat: JsonFormat[AnonymisationLevel.Value] = jsonEnum(
    AnonymisationLevel
  )
  ...
  implicit val TableIdJsonFormat: JsonFormat[TableId] = jsonFormat2(TableId)
